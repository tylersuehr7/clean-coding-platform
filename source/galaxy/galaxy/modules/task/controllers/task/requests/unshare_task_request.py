...
from galaxy.modules.task.services.task.task_service_params import UnshareTaskParams


class UnshareTaskRequest(ApiRequest):
    account_ids = serializers.ListField(child=serializers.UUIDField(), allow_empty=True, max_length=20)

    @classmethod
    def parse(cls, request: Request, path_params: dict = None) -> UnshareTaskParams:
        ...
        unshare: List[Account] = [account_service.get_account_by_id(uid) for uid in api_payload.get("account_ids")]

        return UnshareTaskParams(
            task=task,
            unshare_with=unshare,
        )
